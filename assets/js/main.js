...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  module_default.data("countUp", (target = 0, duration = 1600) => ({
    current: 0,
    started: false,
    init() {
    ...
    run(duration2) {
      ...
      const step = (now) => {
        const progress = Math.min((now - start2) / duration2, 1);
  ...
  module_default.data("siteHeader", () => ({
    scrolled: false,
    mobileOpen: false,
    init() {
      this.onScroll();
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    setTimeout(() => clearInterval(checkVideo), 1e4);
  });
})();
